...
namespace Rubik.Theme.VLC.Controls
{
    ...
    public class VlcMediaPlayer : Control
    {
        ...
        static VlcMediaPlayer()
        {
            ...
            DefaultStyleKeyProperty.OverrideMetadata(typeof(VlcMediaPlayer), new FrameworkPropertyMetadata(typeof(VlcMediaPlayer)));
        }
        ...
        private void SourceChanged()
        {
            ...
            //先停止，后播放
            var media = new LibVLCSharp.Shared.Media(_libVLC, Source);
        ...
        private void UnsubscribeEvents()
        {
            ...
            if (_toggleStatus != null)
                _toggleStatus.Click -= PlayButton_Click;
        ...
        private void PlayButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (_innerPlayer.Media.State == LibVLCSharp.Shared.VLCState.Playing)
                    Task.Run(() => _innerPlayer.SetPause(true));

                if (_innerPlayer.Media.State == LibVLCSharp.Shared.VLCState.Paused)
                    Task.Run(() => _innerPlayer.SetPause(false));

                if (_innerPlayer.Media.State == LibVLCSharp.Shared.VLCState.Ended)
                    _innerPlayer.Play(_innerPlayer.Media);

                return;
            }

            _innerPlayer.Play(new LibVLCSharp.Shared.Media(_libVLC, Source));
        }
        ...
        private void MediaPlayer_Buffering(object sender, LibVLCSharp.Shared.MediaPlayerBufferingEventArgs e)
        {
            BeginInvokeUI(() =>
            {
