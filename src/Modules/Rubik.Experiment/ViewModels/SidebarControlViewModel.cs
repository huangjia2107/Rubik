...
namespace Rubik.Experiment.ViewModels
{
    public class SidebarControlViewModel : BindableBase
    {
        ...
        private void Experiment()
        {
            ...
            var viewType = typeof(ExperimentControl);

            var view = region.Views.FirstOrDefault(v => v.GetType() == viewType);
            if (view != null)
            {
                region.Activate(view);
                _eventAggregator.GetEvent<NavigationContentEvent>().Publish(new NavigationContentPayload { ViewType = viewType, ViewName = "Experiment" });
            }
        }
