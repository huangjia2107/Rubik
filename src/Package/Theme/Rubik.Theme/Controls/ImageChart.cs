...
namespace Rubik.Theme.Controls
{
    ...
    public class ImageChart : System.Windows.Controls.Control
    {
        ...
        {
            if (string.IsNullOrEmpty(key) || points == null || points.Length == 0)
                return;
            ...
            {
                var pointData = new PointData<Pen, long, int> { Value = new Pen(ColorUtil.GetRandomDrawingColor(), 1), Points = points };
        ...
        private void DrawLine(PointData<Pen, long, int> pointData, Graphics g, int width)
        {
            ...
            for (int i = 0; i < pointData.Points.Length; i++)
            {
                ...
                g.DrawLine(pointData.Value, (float)preX, (float)preY, (float)x, (float)y);
